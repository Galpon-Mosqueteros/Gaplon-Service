...
public class CreateBirdCommandFromResourceAssembler {
    public static CreateBirdCommand toCommandFromResource(CreateBirdResource resource) {
        return new CreateBirdCommand(resource.placa(), resource.nombre(), resource.tipo(), resource.color(), resource.peso(),
                resource.estado(), resource.fechaNacimiento(), resource.fechaMuerte());
    }
}
